...
# one-time setup; `perf-replay` is the repeatable A/B step.
...
#   make perf-replay-index-roots   # optional: write roots sidecar for later branches
...
#   make perf-replay-sequencer     # same window, through the block-sync Sequencer
...
# Optional: derive roots sidecar for later VCT fast-sync benchmark branches.
perf-replay-index-roots:
	REPLAY_BIN="$(REPLAY_BIN)" "$(PERF_REPLAY_RUN)" index-roots "$(REPLAY_BIN)"
...
# (one altitude above the direct committer).
perf-replay-worker:
...
# ordered submit to the verifier->state; one altitude above the verifier). Commits
# in Pruned storage mode by default (BASE_SRC must be a pruned snapshot), matching
# the production mainnet config. Optional knobs:
