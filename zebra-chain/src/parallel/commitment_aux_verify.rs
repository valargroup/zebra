...
pub fn header_commitment_is_valid_for_chain_history(
    ...
    match header.commitment(network, height)? {
        ...
        Commitment::ChainHistoryBlockTxAuthCommitment(actual_hash_block_commitments) => {
            ...
            // this block's auth data root (ZIP-244). That still preserves the
            // one-block lag for supplied note commitment roots: `history_tree`
            // is the parent tree, while `auth_data_root` belongs to the current
            // header's block.
            let history_tree_root = history_tree
                .hash()
                .or_else(|| {
