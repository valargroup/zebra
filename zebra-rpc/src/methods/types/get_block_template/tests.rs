...
#[test]
fn coinbase_errors_for_orchard_only_unified_address_after_nu7() {
    let net = nu7_testnet();
    ...
    let error = TransactionTemplate::new_coinbase(&net, nu7_height, &miner_params, Amount::zero())
        .expect_err("Orchard-only unified addresses cannot receive coinbase rewards after NU7");
...
#[test]
fn miner_params_validate_orchard_only_unified_address_at_nu7() {
    let net = nu7_testnet();
    let nu7_height = NetworkUpgrade::Nu7
        .activation_height(&net)
        .expect("NU7 activation height is configured");
    let pre_nu7_height = Height(nu7_height.0 - 1);
    let miner_params = MinerParams::from(Address::Unified(orchard_only_unified_address()));

    miner_params
        .validate_coinbase_receiver(&net, pre_nu7_height)
        .expect("Orchard-only unified addresses are valid before NU7");

    let error = miner_params
        .validate_coinbase_receiver(&net, nu7_height)
        .expect_err("Orchard-only unified addresses are invalid after NU7");
