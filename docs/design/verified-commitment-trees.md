...
authenticated against a header commitment._ Header sync enforces that invariant before it persists
peer-supplied rows: a delivered range confirms only `[start..=end-1]`, so Zebra stores only that
confirmed prefix and deliberately discards the final boundary root until the next overlapped range
redelivers it. A root that cannot be obtained or verified is refused, never guessed: while VCT fast
sync is using verified roots below the last checkpoint, the committer stops and retries rather than
recomputing from a stale frontier (§8).
...
header-sync reactor (zebra-network): validate root count + per-height alignment; reject
   │ unrequested or misaligned roots as MalformedMessage (§8.1)
   ▼
CommitHeaderRange (zebra-state): verify supplied roots against the committed header chain,
   │ store only the confirmed prefix in commitment_roots_by_height (§6)
   ▼
PeerSource (DB-backed reader) -- vct_root(height) --> finalized committer
   │
   ▼
finalized committer: consume confirmed roots, skip recompute --> DB
...
peer GetHeaders { want_tree_aux_roots } -> header-sync reactor -> header-sync driver (zebrad)
   -> ReadRequest::BlockRoots -> committed commitment_roots_by_height index, then confirmed
      header-sync rows in the same index for header-ahead heights (all-or-nothing; §9)
```
...
(2) Header sync requests the per-height roots in-band with the finalized header ranges it already fetches (`want_tree_aux_roots`). Each root-carrying range verifies before persistence and stores only roots confirmed by that same delivery (§4.2, §6). (3) The next forward range overlaps by one height, redelivering the previous boundary header/root so header `H+1` can confirm root `H`; retries self-heal because the unconfirmed boundary row was never stored (§4.3, §8.1).
(4) Each checkpoint block looks up an already-confirmed root, folds it in, and freezes the frontier (§7).
(5) At the last checkpoint height, verify and write the embedded frontier and unfreeze.
(6) Above the last checkpoint height, ordinary semantic verification resumes from the real frontier. A bad/missing root anywhere in the frozen window parks the block and retries in place; it never writes wrong state. Boundary holes are filled by the next overlapped header range, not by an individual roots stream.
...
| **Verify-before-persist** | Authenticating peer-supplied header-sync roots against the node's committed header chain before they are written to `commitment_roots_by_height` (§6). |
| **Confirmed row** | A row in `commitment_roots_by_height` whose roots were confirmed by a delivered successor header. Header sync never stores the final unconfirmed entry of a normal root-carrying range (§6). |
...
new consensus mode. Every supplied root is verified before it is persisted or consumed; a node that cannot
obtain or verify a root falls back to the legacy recompute, bit-identical to today.

## 2. Scope and non-goals

- **In scope:** the consensus-critical commit path (confirmed-root consumption, the frozen-frontier
  failure policy, the checkpoint last checkpoint height), the header-sync transport that carries
  roots in-band, confirmed-root persistence and serving read path, and the persistent
...
ahead of the committer. So they are **carried in-band on the header-sync `Headers` message**
rather than over a separate stream. `GetHeaders` has a `want_tree_aux_roots` flag, and a
...
only to schedule block downloads (§5.4).

Header sync sets `want_tree_aux_roots` on its checkpoint-window range requests. The wire rejects
roots a request opted out of, a root count that does not match the header count, and per-height
misalignment as `MalformedMessage` (§8.1). When a root-carrying range commits via
`CommitHeaderRange`, state verifies the supplied roots against the committed header chain before
writing any of them (§6). For a normal range `[start..=end]`, only `[start..=end-1]` is
confirmed by that delivery, so only those rows are persisted to `commitment_roots_by_height`.
The final entry `end` is checked as the confirmation boundary but is not folded into the header
frontier and is not stored. Only roots below the last checkpoint are ever _consumed_ by the
committer; roots for header-ahead heights above it are confirmed serving data only (§9). The
committer then reads them per height through the `PeerSource` seam.
...
The one coupling to bodies: verifying a root via the ZIP-221 MMR leaf needs the shielded
tx-counts. Header sync receives those counts in the root payload, so it can verify from header
parts before the corresponding bodies are committed; the body committer later consumes only the
confirmed rows.
...
with no separate fetch cursor, fetch-ahead cap, or eviction watermark to manage. The only quirk is
one-height request overlap: after committing a root-carrying range ending at header tip `E`, the
next forward request starts at `E`, anchored at `E-1`, rather than at `E+1`. The redelivered
header/root for `E` must match the stored header chain, and header `E+1` confirms root `E` in the
same delivery. This stores the next confirmed prefix and advances the in-memory header-frontier tree
to the new confirmed tip.

Persisted header-sync rows are therefore confirmed data, not speculative boundary data. They are
bounded above by the header tip and settled below it: each row is **replaced by the verified
serving-index row when its block body commits** (the same atomic batch deletes the header-sync row
and writes the committed row), and header-store rollback also trims rows above the rollback target.
A conflicting header suffix truncates persisted confirmed rows in the same batch, so a retry
rebuilds from local finalized history plus the remaining confirmed rows. Advertised body-size hints
follow the same header-store lifecycle: header reorgs and rollbacks drop stale hints, and committed
block sizes take precedence once the corresponding body is durable.
...
- `PeerSource` - the production default, a **DB-backed reader** (`PeerSource::new(db,
  frontiers)`). Each `vct_root(height)` reads the confirmed root for that height from the
  `commitment_roots_by_height` column family that header sync verified and persisted (§4.2,
  §6). The last checkpoint height frontier is held immutably from the embedded constant, so only
  roots come from the network. `invalidate` **deletes** a rejected root from that column family so
  the next read misses instead of re-reading the same rejected root forever; a verifiable
  replacement arrives when an overlapped header range redelivers and confirms that boundary
  height (§8.1). The earlier in-memory cache variant and its `PeerSourceWriter` are removed;
  proptests fill roots by writing to an ephemeral database through the same header-sync
...
exports a root-writer handle. Header sync writes confirmed roots through `CommitHeaderRange`
...
The persisted roots store no peer identity; peer accountability for bad roots belongs to the
header-sync range that attempted verification (§8.1), preserving the `zebra-state` /
`zebra-network` crate boundary.
...
## 6. Verification - verify-before-persist

Before a supplied root influences consensus state, header-sync range commit confirms it against
the node's own checkpoint-committed headers. The header-only logic lives in
`finalized_state/commitment_aux_verify.rs` and reuses the existing consensus check
`header_commitment_is_valid_for_chain_history` plus `HistoryTree::push_from_parts` - **no new
crypto**.

A block's header commitment binds the history tree _as of its parent_, so the root supplied for
height `H` is folded into a candidate history tree and confirmed when `H+1`'s commitment is
checked against that candidate. Over a delivered range `[start..=end]`, verification checks every
header/root pair in order but folds and persists only `[start..=end-1]`; the final item `end` is
kept as the confirmation boundary and then discarded. The next forward range starts at `end`,
anchored at `end-1`, so root `end` is redelivered fresh by the peer being scored and is confirmed
by header `end+1` in that delivery.

This gives two important properties:

- `commitment_roots_by_height` contains only confirmed data. A boundary row is absent until the
  next overlapped range commits, so the finalized committer either consumes a confirmed row or
  parks with `VctSuppliedRootUnavailable` (§8).
- Every input to a failing range check came from the delivery being scored, or from confirmed local
  state, so root verification failures are attributed to the peer that delivered the bad range,
  including a bad fresh boundary root.

The one exception is the checkpoint last checkpoint height handoff: the last root-carrying range
ends at the last checkpoint height `L`. State verifies `entry_L` against the embedded final
frontiers; on match it folds and stores row `L`, so the body committer's existing handoff check can
consume a confirmed row. On mismatch the range is rejected and nothing is stored.
...
(no one-block lag for the pinned fields - a wrong root is rejected at the entry's own height):

- **Sapling below Heartwood** (`verify_supplied_sapling_root_below_heartwood_from_header`):
  there is no MMR yet, so the header's `FinalSaplingRoot` is compared directly; pre-Sapling the
  root must be the empty-tree root. At/above Heartwood the MMR path authenticates it.
- **Orchard below NU5** (`verify_supplied_orchard_root_below_nu5`): the V1 history leaf
  ...
  header commits to an Orchard root below NU5 - yet the fast path folds the supplied Orchard root
  into the anchor set for every block. The Orchard tree is provably empty there (no Orchard
  actions are allowed), so the supplied root is pinned to the empty-tree root.
- **Ironwood below Nu6_3** (`verify_supplied_ironwood_root_below_nu6_3`): `Nu6_3` is the first
  ...
  header commits to one and the Ironwood tree is provably empty (no Ironwood actions are allowed),
  so the supplied root is pinned to the empty-tree root. At/above `Nu6_3` the MMR path
  authenticates it.

`auth_data_root` below NU5 and shielded tx-count fields below the upgrades that consume them are
still carried and stored because the payload is uniform, but those fields are not consensus-
confirmable before their corresponding history-leaf versions use them.

### 6.2 The auth-data-root cache lock

The NU5+ commitment check trusts a precomputed `AuthDataRoot` carried on
`CheckpointVerifiedBlock` (so the single-threaded committer does not recompute it). Every cached
value is computed from the block by the constructors, so it is correct _by construction_ - but the
public API previously let it be desynced after construction (`pub auth_data_root`, `DerefMut`, both
re-exported). A holder could swap the block while keeping a stale root, and a header matching the
stale root would finalize a block without proving the header binds the block's actual authorizing
data. The (block, auth-data-root) pair is now locked together: `auth_data_root` is `pub(crate)`,
`CheckpointVerifiedBlock` drops `DerefMut`, the one legitimately-post-set field goes through
...
logic. Header sync has already authenticated and persisted rows before the body committer sees
them. For a checkpoint-verified block at `height`:

1. **Fast-root lookup.** `vct.vct_root(height)` returns the confirmed `(sapling, orchard,
   ironwood)` roots, or `None`.
2. **If supplied (fast path):**
   - apply the direct below-Heartwood/below-NU5/below-Nu6_3 checks as defense in depth (§6.1);
   - fold the roots (Sapling, Orchard, and Ironwood) into their anchor sets;
   - skip the frontier recompute; and
   - **freeze** the note-commitment frontier (`vct_frontier_frozen = true`) for non-last checkpoint
     height fast blocks.
3. **Checkpoint last checkpoint height** (when `height` is the last checkpoint height): verify the embedded
   Sapling/Orchard/Ironwood frontiers against this block's confirmed roots (`frontier.root() ==
...
The body committer does not wait for an extra confirming block for normal fast blocks. The
one-block-lag confirmation already happened in the header-range commit that wrote the row. If the body
committer reaches a boundary height before the next overlapped header range has confirmed and
persisted that row, lookup misses and the block parks with `VctSuppliedRootUnavailable` until the
range arrives.
...
- A frozen-frontier height with **no** confirmed root refuses with the retryable
  `VctSuppliedRootUnavailable { height }` error and leaves the database untouched. This is the
  normal short-lived state for a range boundary: the root exists in the previous delivery but was
  intentionally not persisted until an overlapped successor delivery confirms it.
- A root that was confirmed and persisted by header sync but still fails a committer defense-in-depth
  check is **evicted** from its source (so the same rejected root is never re-read) and the commit is
  refused with the same retryable error - not retried against the same rejected root forever, and
...
Outside the frozen window (legacy), a missing root is simply the ordinary legacy recompute -
bit-identical to today. Inside the frozen window, a missing root parks the current checkpoint block
and retries the same commit **in place**, **without resetting the block queue**. Header sync does not
run an individual roots request; liveness comes from the scheduled one-height overlap. If a range
ending at `E` has committed only `[start..=E-1]`, the next forward root-carrying request starts at
`E`, redelivers `entry_E`, and persists it only after header `E+1` confirms it (§4.3, §6). Bodies
lag headers by much more than one range in the common case, so the committer normally finds the
confirmed row by the time it reaches `E`.

This is the safety contract: **a bad, slow, or withholding peer cannot publish a root that
influences state without authentication; after freeze, a later bad or missing delivery never writes
wrong state and does not reset the block queue for root availability.** A height that stays stuck
on a retryable stall past a threshold escalates to an error-level log and the
`state.vct.root.stalled.height` gauge, so a genuinely unservable root surfaces loudly instead of a
silent stall. Counters: `state.vct.root.rejected.count` (evicted after failing verification),
`state.vct.root.unavailable.count` (frozen-frontier hole refused),
`state.vct.root.retry.count` (park-and-retry attempts), and the `state.vct.root.stalled.height`
gauge (raised once a height is stuck past the warn threshold).
...
- **At the wire/reactor boundary**, a peer that sends a malformed root set - wrong count,
  misaligned height, roots on a range that did not request them, or an invalid marker byte - is
  reported through header sync's existing misbehavior path (`report_misbehavior(..,
  MalformedMessage)`), and the range is retried. None of those roots reach state.
- **At range verification**, a well-formed but _wrong_ root fails authentication against the
  header commitment (§6). The range is rejected, nothing from it is stored, and the header-sync
  driver maps the failure to the delivering peer. A bad boundary root in range `[start..=E]` is
  harmless in that delivery because `entry_E` is never stored; if a peer redelivers that bad
  `entry_E` in the next overlapped range, that peer is scored when header `E+1` fails to confirm
  it.

Safety and attribution are both local to the delivery being checked. A lying peer costs one
rejected, scored range; retries self-heal because no unconfirmed boundary data was durably stored,
and an honest overlapped delivery can confirm and persist the missing row.
...
- then appends confirmed header-sync rows from `commitment_roots_by_height` for contiguous heights
  that have headers but no committed body yet - committed roots win on any overlap because they
  are already verified by body commit; and
...
a **complete aligned set** for the served header range (`tree_aux_roots_for_served_header_range`).
A partial set is served as rootless headers, never as a partial root vector - which the
all-or-nothing wire format (§5.4) would reject anyway. This includes a producer whose own best
header tip is the requested range boundary: because that boundary row is absent until the producer
sees a successor range, it may legitimately serve `roots_complete = false`, causing the consumer to
try a peer that is further ahead. The driver maps read errors and wrong responses to a rootless
serve, never wrong data.
...
- **Roots-index CF (lightweight, preferred).** A fast node already verified every root it
  folded in. Persisting confirmed rows into a compact column family (~160 bytes/block, ~550 MB
  for all of Mainnet before compression) lets it serve them without per-height trees, at near-zero
  extra cost. A background task can backfill missing lower ranges by fetching _roots_ (not bodies),
  so even a snapshot-started node becomes a full-range roots server cheaply. This is the targeted
  fix for the §10 serving-availability gap.
...
Protocol hygiene that reduces the failure surface meanwhile: header sync fans each range request
across several peers, and the forward scheduler overlaps root-carrying ranges by one height. A peer
that cannot serve the boundary root because it is at its own tip can return rootless headers while a
peer further ahead returns the complete overlapped range. Confirmed header-ahead rows (§9) widen
the servable range to one below the producer's header tip without per-height trees.
...
- **Increments 0-5 (done):** the fast path proven end-to-end from a local test source - the
  source seam, root verification against headers, the frontier-recompute skip, and the
...
| `state.vct.root.unavailable.count` | frozen-frontier height with no confirmed root; commit refused (retryable) |
...
- **Unit:** the `BlockCommitmentRoots` wire round-trip; the header-sync `Headers`/`GetHeaders`
  ...
  (`reopening_interrupted_fast_sync_without_a_root_source_panics`); the below-NU5 Orchard pin,
  below-Heartwood Sapling check, and below-Nu6_3 Ironwood pin; the header-sync range verifier's
  one-block lag (wrong mid-range root rejected at the confirming height; final range entry checked
  but not folded or stored); the all-or-nothing serving helper
  (`served_header_tree_aux_roots_require_complete_coverage`); confirmed-prefix persistence and
  cleanup on body commit; `PeerSource::invalidate` eviction; and the in-process producer ->
...
- **Header-sync transport:** the header-sync driver tests (`zakura_header_sync_driver_tests`)
  ...
  headers), routing received roots into `CommitHeaderRange`, and restart coverage where
  `roots_tip == header_tip - 1` is treated as covered so scheduling resumes with an overlapped
  request.
- **State persistence:** `CommitHeaderRange` persists only confirmed rows into
  ...
  (`header_range_roots_do_not_overwrite_committed_serving_index_rows`), replaces a header-sync
  row with the verified row when its body commits, treats duplicate range commits as no-ops, and
  truncates the persisted header frontier on header conflicts.
- **Scheduler:** root-carrying forward ranges start at the current header tip and anchor at its
  parent; non-root ranges still start at `tip + 1`; the request count remains clamped by the
  normal byte budget and range limits.
...
| Header-sync root verification and confirmed-prefix folding | `zebra-state/src/service/finalized_state/commitment_aux_verify.rs` |
...
| `BlockRoots` serving read (committed + confirmed header-sync rows) | `zebra-state/src/service.rs` |
| Confirmed rows in `commitment_roots_by_height`, persistence, frontier positioning/rebuild, body-commit/rollback cleanup | `zebra-state/src/service/finalized_state/zebra_db/block.rs`, `.../rollback.rs` |
...
| Header-sync reactor (overlapped request scheduling, serve/receive roots, misbehavior) | `zebra-network/src/zakura/header_sync/reactor.rs`, `.../state.rs` |
| Header-sync driver: serve `BlockRoots`, all-or-nothing helper, route received roots, restart coverage | `zebrad/src/commands/start/zakura/header_sync_driver.rs` |
