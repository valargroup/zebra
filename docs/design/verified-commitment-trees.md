...
existing `zebra-checkpoints` utility already runs against a synced node and produces the
`HEIGHT HASH` checkpoint artifact consumed by `.github/workflows/checkpoint-update.yml`. Extend
that flow with an explicit frontier artifact:
...
exact Sapling, Orchard, and Sprout note-commitment trees. The utility should therefore call a
small `zebra-state` helper that:
...
- immediately validates the generated bytes by parsing them through the same height-checking
  path used for the embedded frontier (`FinalFrontiers::from_bytes` plus
  `parsed.height == Network::Mainnet.checkpoint_list().max_height()`).

The GCP checkpoint-generation workflow should upload `mainnet-frontier.bin` as a separate
artifact for Mainnet runs. `checkpoint-update.yml` should replace the embedded frontier when it
appends new Mainnet checkpoints, and fail closed if Mainnet checkpoints advance but the frontier
artifact is missing or its embedded height does not match the updated checkpoint max height.

Local testing should prove byte compatibility with the node loader:
