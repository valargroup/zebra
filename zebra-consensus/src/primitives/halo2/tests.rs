...
async fn verifier_routes_each_transaction_format_to_the_correct_key() {
    ...
    // NU6.2 and later V5 Orchard bundles route to the fixed V5 key. Nu7 (a future
    // upgrade) guards that "NU6.2 and later" does not silently fall back to the insecure
    // verifier.
    for nu in [
