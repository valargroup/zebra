...
#[test]
fn window_reduction_is_throttled_to_one_per_cooldown() {
    let mut window = download_window();
    ...
    // The first timeout halves the window.
    assert!(window.reduce_outbound_window_after_timeout_throttled(t0, cooldown));
    assert_eq!(window.outbound_request_window, 128);
    // A second timeout within the cooldown leaves concurrency alone (the grace
    // band): a burst of timeouts re-requests every block but cannot collapse it.
    assert!(!window
        .reduce_outbound_window_after_timeout_throttled(t0 + Duration::from_secs(1), cooldown,));
    assert_eq!(window.outbound_request_window, 128);
    // Once the cooldown elapses, a timeout halves the window again.
    assert!(window
        .reduce_outbound_window_after_timeout_throttled(t0 + Duration::from_secs(9), cooldown,));
    assert_eq!(window.outbound_request_window, 64);
}
