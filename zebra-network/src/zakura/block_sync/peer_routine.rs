//! Per-peer routine for Zakura block sync — **start here** to read the subsystem.
//!
...
//! Each connected peer is driven by one task that owns its transport read and runs
//! the download logic inline — there is no central scheduler and no reactor inbound
//! demux.
//!
//! # Where the code lives (reading order)
//!
//! 1. This module — **the core.** One per-peer routine owns its `FramedRecv`,
//!    decodes each stream-6 frame, and runs the want-work fill loop plus the
//!    inbound body/terminator handling in the same task. The download decision
//!    gates on exactly two things — the global in-flight byte budget and the peer's
//!    adaptive outbound request window (its slots) — never on how far ahead of the
//!    committed tip the fetch already is.
...
//! signal (see BS1).
//!
//! # Per-peer task model (the routine IS the per-peer task)
//!
...
use crate::zakura::{
    ...
    Admit, FramedRecv, SessionGuard, SinkReject,
};

/// The per-peer stream-6 ingress guard.
///
/// The transport already applies the per-connection count bucket and frame cap;
/// this guard adds the same payload cap the codec enforces. Type validity is left
/// to the decode stage on purpose: a disallowed or unknown stream-6 type must
/// surface as a `MalformedMessage` misbehavior + protocol reject from the
/// routine's decode-error path (see BS1), rather than a pre-decode guard reject
/// dropping that signal. The block-sync byte budget likewise stays in the
/// routine's reserve/reorder accounting so existing request/retry accounting is
/// not double-counted.
fn block_sync_guard() -> SessionGuard {
...
impl PeerRoutine {
    ...
    /// `run()` inside `spawn_supervised_routine` so a protocol reject cancels the
