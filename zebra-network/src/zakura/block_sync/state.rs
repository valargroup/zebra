...
impl OutstandingBlockRange {
    ...
    /// estimates for every requested height not yet received. Received heights
    /// released their estimate at receipt, so releasing this (on
    /// timeout/disconnect/short response) never double-releases their share.
...
/// lifecycle of one height's in-flight request-estimate reservation:
/// `Reserved(estimate) -> Released`. Received bodies carry no charge here —
/// retention is bounded by the resident look-ahead gate, not the wire budget.
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
