...
use crate::zakura::{
    ApplyItem, BlockApplyResult, BlockSyncAction, BlockSyncEvent, BlockSyncFrontiers,
    BlockSyncHandle, ChainFrontier, Frontier, FrontierChange, FrontierUpdate, ZakuraSyncExchange,
    ZakuraTrace,
};
use crate::BoxError;
...
pub(crate) async fn run_scenario(
    ...
    let apply_rx = handle
        .take_apply_queue()
        .ok_or("reactor did not expose an applyQ receiver")?;
    ...
    tasks.push(spawn_commit_driver(
        handle.clone(),
        apply_rx,
        apply.clone(),
        scenario.commit,
...
/// Answers the reactor's `QueryNeededBlocks` from the corpus (the producer seam) and
/// ignores serving queries (a synced leecher does not serve here).
...
/// Drains the applyQ, applies each body through the mock commit frontier, reports the
/// durable frontier back to the reactor (advancing the floor), and publishes the
/// committed tip.
fn spawn_commit_driver(
    handle: BlockSyncHandle,
    ...
    tokio::spawn(async move {
        // Mirror the real Committer's per-height apply-epoch guard: the Sequencer
