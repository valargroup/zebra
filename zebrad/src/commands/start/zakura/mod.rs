...
/// queue, so fallback must be a commit barrier: once yielded, the block-sync
/// driver starts no new applies, and the watchdog waits for in-flight applies
/// to finish before resuming legacy sync. The Zakura reactors stay alive
/// throughout; only bulk body applies are gated.
#[derive(Debug)]
