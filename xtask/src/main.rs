...
const DEFAULT_FEATURES: &str = "default-release-binaries p2p-tracing";
const DEFAULT_UBUNTU_IMAGE: &str = "ubuntu:22.04";
const DEFAULT_RUST_VERSION: &str = "1.89.0";
...
fn print_usage(output: &mut impl fmt::Write) -> fmt::Result {
    ...
    writeln!(
        output,
        "Builds an Ubuntu 24.04 zebrad release binary using Docker,"
    )?;
    writeln!(
        output,
        "enables `{DEFAULT_FEATURES}`, and writes it to target/ubuntu/{OUTPUT_BINARY_NAME}.",
    )
}
