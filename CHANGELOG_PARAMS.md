...
| --- | --- | --- | --- | --- |
| Block-sync look-ahead resident accounting (stage-aware) | `zebra-network/src/zakura/block_sync/admission.rs` | flat `(reorder + applying + input + reserved) × DESERIALIZED_MEM_FACTOR` → applying + sequencer-input `× factor`, reorder backlog `×1` (wire-retained), above-floor reservations `×0` | [ZCA-742](https://linear.app/zcale/issue/ZCA-742) | The flat multiple over-counted the wire-retained reorder backlog (~1× resident) and unreceived reservations (0×), throttling effective look-ahead depth up to ~4× below the configured budget in the reorder-/reservation-heavy burst states this budget exists to deepen. Charge only the decoded pipeline stages at the multiple. |
