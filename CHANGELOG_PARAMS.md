...
| --- | --- | --- | --- | --- |
| `PARALLEL_HASH_THRESHOLD` | `zebra-chain/src/parallel/batch_frontier.rs` | new → `16` | [#387](https://github.com/valargroup/zebra/pull/387) | Minimum per-block note count below which off-committer rayon parallel hashing is skipped in favor of inline serial hashing, because below the crossover the fork-join overhead outweighs the parallelism (see the `precompute_threshold` bench). |
