...
| --- | --- | --- | --- | --- |
| `BUDGET_AUDIT_INTERVAL` | `zebra-network/src/zakura/block_sync/sequencer_task.rs` | _(new)_ → `1s` | _(this PR)_ | Throttle the sequencer's byte-budget drift audit to at most once per interval. The audit re-derives the reserved total via an O(pending + in_flight) work-queue scan; running it on every body/control event (as `publish_view` did) is quadratic in the backlog and stalls the commit pipeline. The published view uses the budget's O(1) running counter, so the scan is only a drift metric. |
