...
impl WriteBlockWorkerTask {
    ...
    pub fn run(mut self) {
        ...
        // One-block look-ahead for VCT fast sync: the current block's supplied
        // roots are authenticated by the successor's header commitment, so the
        // writer buffers one checkpoint block ahead when possible.
