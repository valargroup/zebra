...
/// / `orchard_start` / `ironwood_start` (the tree `count`s the block will commit
/// at) — so the heavy hashing overlaps the *current* block's commit on otherwise
/// idle cores. The committer then only applies the precomputed subtree roots. If
/// the precompute is stale (its `start_size` no longer matches the tree), the
/// committer falls back to inline hashing, so this is purely a scheduling
/// optimization.
///
                        ...
                        rayon::in_place_scope_fifo(|scope| {
                            ...
                            timed_commit_phase!(
                                ...
                                note_commitment_trees
                                    .update_trees_parallel_precompute(&block, note_precompute)
                            )
                        })
                    });
