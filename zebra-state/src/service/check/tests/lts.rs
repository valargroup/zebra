...
fn claimed_payout_rejects_over_claim_in_disbursement_window() {
    ...
    let (block, spent_utxos) = block_with_coinbase_and_fee(&network, start, output, Amount::zero());

    let err =
        check_claimed_lts_payout(&network, &chain, &finalized.db, start, &block, &spent_utxos)
...
fn claimed_payout_enforces_zip235_minimum_with_fees() {
    ...
    assert_eq!(
        Amount::<NonNegative>::try_from(minimum).unwrap(),
        expected_minimum
    );
    assert_eq!(
