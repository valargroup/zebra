...
- Bumped the state database format to 28 during upgrade, extending value-pool
  disk serialization with an Ironwood slot, backfilling empty Ironwood tree,
  value pool, and index data, then rebuilding stored history tree entries so
  they use the Ironwood-capable entry size.
