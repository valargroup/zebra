...
- **Sync Confidence** (`sync-confidence.yml`): Full-validation sync of fixed pre/post-NU6.2 mainnet block ranges on merge to `ironwood-main` (and manual dispatch), to catch sync-breaking consensus regressions
...
- **Sync confidence snapshots** (`sync-confidence-snapshots.yml`): Manually builds the cached-state snapshot disks the Sync Confidence job restores from (rerun after a database format version bump)
