...
- **Sync Confidence** (`sync-confidence.yml`): Full-validation sync of fixed pre/post-NU6.2 mainnet ranges on merge to `ironwood-main` (and manual dispatch), on ephemeral DigitalOcean droplets (nyc3) restoring cached state from Spaces. See `docs/sync-confidence-do-setup.md`.
